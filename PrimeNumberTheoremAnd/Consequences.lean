...
lemma exists_between_count {p} (a b : ℕ) (hab : a < b) (h : a.count p < b.count p)
    : ∃(x : ℕ), p x ∧ a ≤ x ∧ x < b := by
...
lemma prime_in_gap (a b : ℝ) (ha : 0 < a)
  ...
  · calc
      ...
      _ ≤ w := by norm_cast
  · by_contra h
    have b_lt_w : b < w := lt_of_not_le h
    have : ⌊b⌋₊ + 1 ≤ w := by
      have : a < b := by
        by_contra h
        have := le_of_not_lt h
      ...
      have : ⌊b⌋₊ < w := (floor_lt this).mpr b_lt_w
      linarith
    linarith
